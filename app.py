...
file_key = f"{uploaded.name}:{len(file_bytes)}:{report_date.isoformat()}"
if st.session_state.get("forecast_file_key") != file_key:
    st.session_state["forecast_file_key"] = file_key
    st.session_state["deal_judgements"] = prepare_scenario_deals(result.cleaned, report_date)
...
def deals_for_period(deals, period, quarterly_period=False):
    close_dates = pd.to_datetime(deals["Close Date"], errors="coerce")
    if quarterly_period:
        return deals[close_dates.dt.to_period("Q").astype(str) == str(period)]
    return deals[close_dates.dt.to_period("M").astype(str) == str(period)]

deal_columns = ["見込み区分", "商談名", "商談MRR", "Close Date", "初回商談日", "フェーズ", "主リードソース", "次のステップ & 状況"]
pipeline_columns = ["商談名", "商談MRR", "Close Date", "初回商談日", "フェーズ", "主リードソース", "次のステップ & 状況"]
...
with top:
    ...
    pcols = st.columns(4)
    for col, ((_, row), (_, period, is_q)) in zip(pcols, zip(pipeline_summary.iterrows(), period_specs)):
        ...
        with col.popover("Pipeline対象案件を見る", use_container_width=True):
            target_pipeline = deals_for_period(pipeline_deals, period, is_q)
            cols = [c for c in pipeline_columns if c in target_pipeline.columns]
            st.dataframe(target_pipeline[cols].sort_values("商談MRR", ascending=False), use_container_width=True, hide_index=True)
    ...
    for col, (label, period, is_q) in zip(rcols, period_specs):
        ...
        with col.popover("着地対象案件を見る", use_container_width=True):
            target_scenarios = deals_for_period(full_deals, period, is_q)
            cols = [c for c in deal_columns if c in target_scenarios.columns]
            st.dataframe(target_scenarios[cols].sort_values("商談MRR", ascending=False), use_container_width=True, hide_index=True)
...
if lead_monthly.empty:
    ...
    lead_select = alt.selection_point(fields=["月", "リードソース"], name="lead_select", on="click", clear="dblclick")
    heatmap = alt.Chart(lead_monthly).mark_rect(cornerRadius=4).encode(
        ...
        opacity=alt.condition(lead_select, alt.value(1), alt.value(.88)),
        tooltip=["月:N", "リードソース:N", "アポ数:Q", "注力案件数:Q"],
    ).add_params(lead_select)
    ...
    lead_event = st.altair_chart(
        (heatmap + labels).properties(height=max(260, 42 * lead_monthly["リードソース"].nunique())),
        use_container_width=True,
        on_select="rerun",
        selection_mode="lead_select",
        key="lead_source_heatmap",
    )
    selected_lead = []
    try:
        selected_lead = lead_event.selection.get("lead_select", [])
    except (AttributeError, KeyError, TypeError):
        pass
    if selected_lead:
        selected_month = selected_lead[0].get("月")
        selected_source = selected_lead[0].get("リードソース")
        source_deals = lead_source[(lead_source["月"] == selected_month) & (lead_source["リードソース"] == selected_source)]
        st.markdown(f"#### {selected_month}｜{selected_source}")
        st.caption("クリックしたセルの正式なリードソース名と対象案件です。")
        source_cols = [c for c in ["商談名", "商談MRR", "初回商談日", "フェーズ", "注力案件", "主リードソース", "次のステップ & 状況"] if c in source_deals.columns]
        st.dataframe(source_deals[source_cols].sort_values("初回商談日", ascending=False), use_container_width=True, hide_index=True)
